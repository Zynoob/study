...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    if (from.name == "admin_buildpaper" || from.name == null) {
      next();
    } else {
      next("/admin/buildpaper");
    }
  },
  created() {
  ...
  methods: {
    ...
    toTeacherPage() {
      if (confirm("是否放弃当前创建的试卷？")) {
        clearSessionStorage();
        this.$router.replace("/admin/buildpaper");
      } else {
        return false;
      }
    ...
    submit() {
      ...
      console.log(obj);
      this.$mypost("/api/admin/buildPaper.php", {
        paper: obj
      })
        .then(data => {
          // console.log(data);
